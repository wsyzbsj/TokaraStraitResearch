...
u_used = u[0, 0, np.argwhere(lat > lat_min)[0][0]:np.argwhere(lat < lat_max)[-1][0],np.argwhere(lon > lon_min)[0][0]:np.argwhere(lon < lon_max)[-1][0]]
v_used = v[0, 0, np.argwhere(lat > lat_min)[0][0]:np.argwhere(lat < lat_max)[-1][0],np.argwhere(lon > lon_min)[0][0]:np.argwhere(lon < lon_max)[-1][0]]
dataset.close()
# 创建规则网格
print('创建网格')
...
# 插值到规则网格
points_data = np.column_stack((lon_used, lat_used))
