...
# 绘制断面线
ax_main.plot(lon_list, lat_list, 'r-', linewidth=2, transform=ccrs.Geodetic())

# 标记起点和终点
ax_main.plot(point_start_loc[1], point_start_loc[0], 'go', markersize=5, transform=ccrs.PlateCarree(), label='起点')
...
ax_zoom.plot(point_end_loc[1], point_end_loc[0], 'ro', markersize=10,
        transform=ccrs.PlateCarree(), label='终点')

# 添加标注
ax_zoom.text(point_start_loc[1] + 0.1, point_start_loc[0], f'起点\n{point_start_loc[0]}°N, {point_start_loc[1]}°E',
...
# 调整布局并保存
plt.savefig('output/研究区域_局部放大.png', dpi=600)
plt.show()