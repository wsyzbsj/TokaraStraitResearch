...
if __name__ == "__main__":
    ...
    # === 2. 读取数据 ===
    spring_data = np.load('output/HYCOM/'+current_section+'/ave_spring_salinity_section.npz')
    summer_data = np.load('output/HYCOM/'+current_section+'/ave_summer_salinity_section.npz')
    autumn_data = np.load('output/HYCOM/'+current_section+'/ave_autumn_salinity_section.npz')
    winter_data = np.load('output/HYCOM/'+current_section+'/ave_winter_salinity_section.npz')

    # 设置最大最小值
    max_y_loc = np.min([np.argwhere(spring_data['y']<=max_y)[-1,0],len(spring_data['y'])-1])
    ...
    plt.suptitle('Water salinity at '+current_section+' section', fontsize=20)
    ...
    cbar.set_label('Water Salinity (PSU)', fontsize=12, rotation=270, labelpad=15)

    plt.savefig('output/HYCOM/'+current_section+'/季节盐度断面.png', dpi=600)
    plt.show()
    plt.close()