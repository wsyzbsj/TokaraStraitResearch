...
# 判断输出目录是否存在
if os.path.exists('output_compare_hycom_adcp'):
    shutil.rmtree('output_compare_hycom_adcp')
os.makedirs('output_compare_hycom_adcp')
...
# 筛选区域内的网格
lon_max = max(lon_adcp)+0.25
lon_min = min(lon_adcp)-0.25
lat_max = max(lat_adcp)+0.25
lat_min = min(lat_adcp)-0.25
lat_grid_step = 0.04
lon_grid_step = 0.08
# 插值
# 创建目标网格
num_lon = int((lon_max - lon_min) / lon_grid_step) + 1
num_lat = int((lat_max - lat_min) / lat_grid_step) + 1
...
# 读取选定区域数据
lon_used = lon[np.argwhere(lon > lon_min)[0][0]:np.argwhere(lon < lon_max)[-1][0]]
lat_used = lat[np.argwhere(lat > lat_min)[0][0]:np.argwhere(lat < lat_max)[-1][0]]
u = dataset.variables['water_u'][:]
v = dataset.variables['water_v'][:]
depth = dataset.variables['depth'][:]
u_used = u[0,:,np.argwhere(lat > lat_min)[0][0]:np.argwhere(lat < lat_max)[-1][0],
           np.argwhere(lon > lon_min)[0][0]:np.argwhere(lon < lon_max)[-1][0]] # (深度40,纬度25,经度43)
v_used = v[0,:,np.argwhere(lat > lat_min)[0][0]:np.argwhere(lat < lat_max)[-1][0],
           np.argwhere(lon > lon_min)[0][0]:np.argwhere(lon < lon_max)[-1][0]] # (深度40,纬度25,经度43)
...
for i_point in range(len(lat_adcp)): # 观测点
    distance.append([])
    print('正在处理格点：'+str(i_point+1)+'/'+str(len(lon_adcp)))
    for i_lat in range(len(lat_used)): # 网格
        for i_lon in range(len(lon_used)):
            distance[i_point].append(gd((lat_adcp[i_point], lon_adcp[i_point]), (lat_used[i_lat], lon_used[i_lon])).km)   # 计算距离
distance = np.array(distance)
# 计算离观测点最近的网格下标（筛选后的下标），从depth_used中获取
min_index = []
for i_point in range(len(lat_adcp)): # 观测点
    index = np.argmin(distance[i_point])
    i_lat = index // len(lon_used)  # 计算纬度索引
    i_lon = index % len(lon_used)   # 计算经度索引
    min_index.append([i_lat, i_lon])
...
for i_point in range(len(min_index)):  # 插值:(目标点/一维数组,x,y) 获取对应深度的速度
    u_hycom.append(np.interp(depth_adcp,depth[:],u_used[:,min_index[i_point][0],min_index[i_point][1]],left=np.nan,right=np.nan)) # depth_used (31776,68)
    v_hycom.append(np.interp(depth_adcp,depth[:],v_used[:,min_index[i_point][0],min_index[i_point][1]],left=np.nan,right=np.nan))
u_hycom = np.array(u_hycom)  # (151,67) 格点，深度
v_hycom = np.array(v_hycom)  # (151,67)
u_hycom *= 100
v_hycom *= 100
...
for i_lev in range(len(depth_adcp)): # 深度层
    ...
    cax_height = 0.48  # colorbar 高度
    cax_left = 0.92  # 与地图左侧对齐
    cax_bottom = 0.21  # 在地图下方，留出5%的间距
    ...
    # l1 = ax.quiver(lon_adcp, lat_adcp,u_adcp[i_lev,:],v_adcp[i_lev,:],scale=1000,width=0.001,color='blue', label='走航ADCP实测结果')
    # l2 = ax.quiver(lon_adcp, lat_adcp,u_hycom[:,i_lev],v_hycom[:,i_lev],scale=1000,width=0.001,color='red', label='HYCOM分析数据结果')
    # ax.legend((l1,l2),('走航ADCP实测结果','HYCOM分析数据结果'),loc='upper right')
    q1 = ax.quiver(lon_adcp, lat_adcp, u_adcp[i_lev, :], v_adcp[i_lev, :], scale=1000, width=0.001, color='blue')
    q2 = ax.quiver(lon_adcp, lat_adcp, u_hycom[:, i_lev], v_hycom[:, i_lev], scale=1000, width=0.001, color='red')

    # 添加自定义图例（箭头）
    ax.quiverkey(q1, 0.85, 0.92, 20, '走航ADCP实测', coordinates='axes', labelpos='E', fontproperties={'size': 10})
    ax.quiverkey(q2, 0.85, 0.87, 20, 'HYCOM分析数据', coordinates='axes', labelpos='E', fontproperties={'size': 10})
    ax.set_title('船载ADCP与HYCOM Analysis数据'+str_depth_adcp+'m深度流速结果对比')
    plt.savefig('output/compare_adcp/船载ADCP与HYCOM Analysis数据'+str_depth_adcp+'m深度流速结果对比.png', dpi=600, bbox_inches='tight')
    plt.close()