...
if __name__ == "__main__":
    ...
    # === 2. 读取数据 ===
    spring_data = np.load('output/HYCOM/'+current_section+'/ave_spring_water_temp_section.npz')
    summer_data = np.load('output/HYCOM/'+current_section+'/ave_summer_water_temp_section.npz')
    autumn_data = np.load('output/HYCOM/'+current_section+'/ave_autumn_water_temp_section.npz')
    winter_data = np.load('output/HYCOM/'+current_section+'/ave_winter_water_temp_section.npz')
    ...
    plt.suptitle('Water temperature at '+current_section+' section', fontsize=20)
    ...
    mesh = ax1.contourf(
        ...
        vmin=vmin,
        vmax=vmax,
        levels=np.arange(vmin, vmax+0.05, 0.20),
        zorder=0
    )
    ...
    cbar.set_label('Water Temperature (℃)', fontsize=12, rotation=270, labelpad=15)

    # 保存
    plt.savefig('output/HYCOM/'+current_section+'/季节水温断面.png', dpi=600, bbox_inches='tight')
    plt.show()
    plt.close()